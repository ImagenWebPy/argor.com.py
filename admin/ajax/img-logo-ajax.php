...
if (!empty($_FILES["img_logo"]['name'])) {
    ...
    if ($tipo != 'image/jpg' && $tipo != 'image/jpeg' && $tipo != 'image/png' && $tipo != 'image/gif') {
        ...
        #eliminamos el archivo del logo viejo
        $sqlLogo = "SELECT img FROM banner WHERE id = $id";
        $db->setQuery($sqlLogo);
        $logo = $db->loadObject();
        #eliminamos el archivo
        unlink($carpeta . $logo->logo);
        #actualizamo el nombre del archivo
//        $updateLogo = "UPDATE logo SET logo = '" . limpia_espacios(limpiar($nombre)) . "'"
//                . "WHERE id = 1";
//        db::getInstance()->exec($updateLogo);
        #subimos el archivo
//        $src = $carpeta . limpia_espacios(limpiar($nombre));
//        move_uploaded_file($ruta_provisional, $src);
        ...
        //reducimos el tamaño y redimensionamos la imagen
        //tamaño de ancho
